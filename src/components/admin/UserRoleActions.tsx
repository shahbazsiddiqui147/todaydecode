...
import { updateUserRole, approveUser, deleteUser } from "@/lib/actions/admin-actions";
import {
    ChevronDown,
    UserCheck,
    ShieldAlert,
...
export default function UserRoleActions({ userId, currentRole, isApproved }: UserRoleActionsProps) {
        ...
        <div className="flex items-center justify-end space-x-2">
            {!isApproved && (
                <Button
                    ...
                    disabled={loading}
                    className="h-9 px-4 text-[10px] font-black uppercase tracking-widest text-[#22D3EE] border-[#22D3EE] hover:bg-[#22D3EE] hover:text-[#0F172A] transition-all bg-transparent"
                >
            ...
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56 bg-white dark:bg-[#0D1425] border-slate-200 dark:border-[#1E293B] rounded-2xl shadow-2xl p-2">
                    ...
                    <div className="px-3 py-2 mt-1 border-t border-slate-100 dark:border-[#1E293B]">
                        <span className="text-[9px] font-black uppercase tracking-[0.2em] text-red-400">Destructive Actions</span>
                    </div>
                    <DropdownMenuItem
                        onClick={handleDelete}
                        disabled={loading}
                        className="flex items-center space-x-3 px-3 py-2.5 rounded-xl cursor-pointer hover:bg-red-50 dark:hover:bg-red-900/20 text-red-600 transition-colors"
                    >
                        <Trash2 className="h-4 w-4" />
                        <span className="text-xs font-black uppercase tracking-tight">Purge Personnel</span>
                    </DropdownMenuItem>
